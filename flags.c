...
static inline char	*_do_flags_CS(char *r, t_spec *s, t_types typ, void *val)
{
	if (!r && (s->mpflg == TRUE && s->minprec < 6))
		// print "" if small minprec  TODO:check redudnant
		r = ft_substr("", 0, 0);
	else if (!r && (s->mwflg || (s->mpflg == TRUE && s->minprec >= 6)))
		// TODO why is this redundant, double check
		r = ft_substr("(null)", 0, 6);
	// print "(null)" if enuf minprec + any minw
	else if (r)
		// create new string for char
...
/* Sets negative sign char and flag for printing */
static inline void	_do_neg_value(long long *value, t_spec *s)
{
	s->sch = "-";
	s->signflg = TRUE;
	*value *= -1;
